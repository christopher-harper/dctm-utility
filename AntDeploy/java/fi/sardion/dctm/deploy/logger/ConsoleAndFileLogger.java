...
/**
 * <p>
 * A simple logger that writes the same output as is written to the console to a
 * temporary file.
 * <ul>
 ...
 * <li>In the opening dialog select the Main tab and add:
 * <code>-logger fi.sardion.dctm.deploy.logger.ConsoleAndFileLogger -verbose</code>
 * to the Arguments text box.</li>
 * <li>Look for: <code>(Sending/Sent) console output to file {file_path}</code>
 * in order to locate the created log file.</li>
 * </ul>
 * </p>
 * 
...
public class ConsoleAndFileLogger extends DefaultLogger {
    ...
    /**
     * Event that is triggered when the build ends. Writes a message to the
     * console about the log file location.
     * <ul>
    ...
    /**
     * Event that is triggered when the build starts. Writes a message to the
     * console about the log file location.
     * <ul>
	...
	try {
	    ...
	    this.out = new PrintStream(new BufferedOutputStream(
		    new FileOutputStream(getLogFile())));
	    System.out.println(String.format(
		    "Sending console output to file %s.", //$NON-NLS-1$
		    new Object[] { getLogFile().getAbsolutePath() }));
