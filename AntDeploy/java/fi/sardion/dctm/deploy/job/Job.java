...
public class Job extends Persistent {
	...
	// /**
	// * Create or alter the job object.
	// * <ul>
	// * <li>Created: 18 Sep 2010 16:45:18</li>
	// * <li>Author: Christopher Harper, account: dmadmin</li>
	// * </ul>
	// *
	// * @since %since%
	...
	// final String qualification = new StringBuilder(100).append(
	//		        DCTMTask.DM_JOB).append(" where ").append( //$NON-NLS-1$
	// DCTMTask.OBJECT_NAME)
	//		        .append(" = '").append(getJobName()) //$NON-NLS-1$
	// .append('\'').toString();
	// IDfSysObject job = (IDfSysObject) session
	// .getObjectByQualification(qualification);
	// if (job == null) {
	// log(String
	// .format(
	//			            "Qualification %s din't return a the job. Creating a new one.", //$NON-NLS-1$
	// new Object[] { qualification }));
	// job = (IDfSysObject) session.newObject(DCTMTask.DM_JOB);
	// job.setObjectName(getJobName());
	//		    job.grant(DCTMTask.DM_WORLD, IDfACL.DF_PERMIT_READ, ""); //$NON-NLS-1$
	// // job.link(Job.JOB_FOLDER);
	// setReaplySettings(String.valueOf(true));
	// } else {
	//		    log(String.format("Found job with qualification %s.", //$NON-NLS-1$
	// new Object[] { qualification }));
	// }
	// if (reaplySettings()) {
	// if (!job.isDirty()) {
	//			log(String.format("Modifying existing job %s values.", //$NON-NLS-1$
	// new Object[] { getJobName() }));
	// }
	// job.setString(DCTMTask.METHOD_NAME, getMethodName());
	// job.setTime(DCTMTask.EXPIRATION_DATE, getExpirationDate());
	// job.setBoolean(DCTMTask.INACTIVATE_AFTER_FAILURE,
	// getInactivateAfterFailure());
	// job.setInt(DCTMTask.MAX_ITERATIONS, getMaxIterations());
	// job.removeAll(DCTMTask.METHOD_ARGUMENTS);
	// for (final String argument : getMethodArguments()) {
	// job.appendString(DCTMTask.METHOD_ARGUMENTS, argument);
	// }
	// job.setBoolean(DCTMTask.PASS_STANDARD_ARGUMENTS,
	// getPassStandardArguments());
	// job.setInt(DCTMTask.RUN_INTERVAL, getRunInterval());
	// job.setInt(DCTMTask.RUN_MODE, getRunMode());
	// job.setTime(DCTMTask.START_DATE, getStartDate());
	// job.setTime(DCTMTask.A_NEXT_INVOCATION, getStartDate());
	// job.setString(DCTMTask.TARGET_SERVER, getTargetServer());
	// job.setTitle(getTitle());
	// job.setBoolean(DCTMTask.IS_INACTIVE, false);
	// job.save();
	// if (isVerbose()) {
	// log(
	// String
	// .format(
	//			                        "########## JOB OBJECT DUMP START ##########\n%s\n########### JOB OBJECT DUMP END ###########", //$NON-NLS-1$
	// new Object[] { job.dump() }),
	// Project.MSG_INFO);
	// }
	// }
	...
	// log(String
	// .format(
	//		                "This job %s was only defined for repositories %s and the current repository is %s.", //$NON-NLS-1$
